								 ...
								 AND clave ='".hash('sha256', $_POST['pass'])."'";				 
								 	try {
						                $datos2=DB::select($sql);
				          					...
				          					$request->session()->set('correo', $datos2[0]->login);
								            $request->session()->set('nombre', $datos2[0]->login);
								            $request->session()->set('id', $datos2[0]->idAdministrador);
